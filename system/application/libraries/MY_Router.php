	...
	function check_method()
	{
		...
		if (class_exists($class))
		{
				...
				&& ! in_array(strtolower($this->method), array_map('strtolower', get_class_methods($class))))
			{
                ...
                $error_controller = APPPATH.'controllers/'.$this->fetch_directory().$this->error_controller.EXT;
                if (! is_file($error_controller))
                {
                    $error_controller = APPPATH.'controllers/'.$this->error_controller.EXT;
                }
                include($error_controller);
			}
		}
	}
}
